...
parser.add_argument('--bs', default=64, type=int, help='batch_size') #128
parser.add_argument('--lr', default=0.01, type=float, help='learning rate')
parser.add_argument('--resume', '-r', action='store_true', help='resume from checkpoint')
opt = parser.parse_args()


print(torch.cuda.is_available())  #我加的
...
transform_train = transforms.Compose([
    ...
    transforms.ToTensor(),
])  #RandomCrop对图像进行随机裁剪，RandomHorizontalFlip是依概率p水平翻转图像

transform_test = transforms.Compose([
    transforms.TenCrop(cut_size),
    transforms.Lambda(lambda crops: torch.stack([transforms.ToTensor()(crop) for crop in crops])),
]) #TenCrop是上下左右中心裁剪后翻转图片。自定义自己的transform策略方法就是使用transforms.Lambda去封装自定义函数。

trainset = CK(split = 'Training', fold = opt.fold, transform=transform_train)
#trainloader = torch.utils.data.DataLoader(trainset, batch_size=opt.bs, shuffle=True, num_workers=1)#num_workers是加载多线程，报错，所以我去掉
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        #print("inputs.shape",inputs.shape," targets:",targets.shape)#我加的
        outputs = net(inputs)
        ...
        #train_loss += loss.data[0] #我注解掉的，运行出错
        train_loss+=loss.item() #我加的，代替上面那一行
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        if use_cuda:
            inputs, targets = inputs.cuda(), targets.cuda()
        #inputs, targets = Variable(inputs, volatile=True), Variable(targets)#出错，我注释掉了
        inputs, targets = Variable(inputs), Variable(targets)
        outputs = net(inputs)
        outputs_avg = outputs.view(bs, ncrops, -1).mean(1)  # avg over crops

        loss = criterion(outputs_avg, targets)
        #PrivateTest_loss += loss.data[0] #这句运行出错，我注解掉。
        PrivateTest_loss +=loss.item() #代替上面那一句，我加的。
