...
class CK(data.Dataset):
    ...
    def __init__(self, split='Training', fold = 1, transform=None):
        ...
        for j in range(len(test_number)): #range原来是xrange   
            for k in range(test_number[j]): #range原来是xrange
        ...
        for i in range(number): #range原来是xrange
        ...
        if self.split == 'Training':
            self.train_data = []
            self.train_labels = []
            for ind in range(len(train_index)):#range原来是xrange
            ...
            self.test_data = []
            self.test_labels = []
            for ind in range(len(test_index)):#range原来是xrange
